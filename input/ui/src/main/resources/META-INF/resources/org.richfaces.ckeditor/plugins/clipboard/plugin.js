 ...
 */
//META-INF/resources/org.richfaces.ckeditor/plugins/preview
(function()
{
	...
	function getClipboardData( evt, mode, callback )
	{
		...
		if (esArchivo(evt)){
			...
			if (evt && evt.data && evt.data.$ && evt.data.$.preventDefault) {
				evt.data.$.preventDefault();	
			}	//
				//	
			procesarPasteArchivo(evt, function (data) {
				callback(data);
			});
			return;
		}
		...
		if ( mode == 'text' && evt && evt.data && evt.data.$.clipboardData )
		{
		...
		function esArchivo(evt) {
			...
			var clipboardData = getClipboardEventData(evt);
        ...
        function procesarPasteArchivo (evt, callback) {
		...
		function getClipboardEventData(evt) {
			
			var clipboardData = null;
			if (evt && evt.data && evt.data.$  && evt.data.$.clipboardData) {
				clipboardData = evt.data.$.clipboardData;
			}
